...
const BrandEdit = (props: Props) => {
  ...
  const updateBrand = () => {
    ...
    brandService.update(props.brand._id, name).then(() => {
      props.exitEditMode()
    }).catch((errorMessage) => {
    ...
    <TightModal
      ...
      <GridLayout>
        <IonCol size="12">
          <IonList>
            <IonItem>
              <IonInput autofocus value={name} onIonChange={e => { setName(e.detail.value || ''); setError('') }} onKeyUp={e => { if (e.key === 'Enter') updateBrand(); }} />
            </IonItem>
